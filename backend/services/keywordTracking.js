...
export default async function keywordTracking(tracking){
    try {
        

        let result;

        for(let attemp = 1; attemp<= 2; attemp++){
            result = await rankTracker(tracking.keyword, tracking.domain)
            if(result.success && result.data.totalResultsScanned > 0) break;
            if(attemp < 2) await new Promise((r) => setTimeout(r, result.success ? 3000 : 5000))
        }
